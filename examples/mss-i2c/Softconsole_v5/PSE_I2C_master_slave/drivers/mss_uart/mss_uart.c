 ...
 * All rights reserved.
 *
 * PolarFire SoC(MPFS) Microcontroller Subsystem MMUART bare metal software
 * driver implementation.
 *
 *
 * SVN $Revision: 11263 $
 * SVN $Date: 2019-05-16 12:21:21 +0530 (Thu, 16 May 2019) $
 */
...
#define MSS_UART0_LO_BASE           (MSS_UART_TypeDef*)0x20000000
#define MSS_UART1_LO_BASE           (MSS_UART_TypeDef*)0x20100000
#define MSS_UART2_LO_BASE           (MSS_UART_TypeDef*)0x20102000
#define MSS_UART3_LO_BASE           (MSS_UART_TypeDef*)0x20104000
#define MSS_UART4_LO_BASE           (MSS_UART_TypeDef*)0x20106000

#define MSS_UART0_HI_BASE           (MSS_UART_TypeDef*)0x28000000
#define MSS_UART1_HI_BASE           (MSS_UART_TypeDef*)0x28100000
#define MSS_UART2_HI_BASE           (MSS_UART_TypeDef*)0x28002000
#define MSS_UART3_HI_BASE           (MSS_UART_TypeDef*)0x28004000
#define MSS_UART4_HI_BASE           (MSS_UART_TypeDef*)0x28006000
...
#define MSS_UART_INTR_LOCAL_AND_PLIC     3
...
static void enable_irq(mss_uart_instance_t * this_uart);
static void disable_irq(mss_uart_instance_t * this_uart);
...

{
    ...
    {
        if((MSS_UART_INTR_LOCAL == this_uart->intr_type) )
        {
            /* Enable local interrupt UART instance . */
            __enable_local_irq((int8_t)MMUART0_E51_INT);
            PLIC_DisableIRQ(MMUART0_PLIC_77);
        }
        else if((MSS_UART_INTR_PLIC == this_uart->intr_type))
        {
            /* Enable UART instance interrupt in PLIC. */
            PLIC_EnableIRQ(MMUART0_PLIC_77);
            __disable_local_irq((int8_t)MMUART0_E51_INT);
        }

        else if((MSS_UART_INTR_LOCAL_AND_PLIC == this_uart->intr_type))
        {
            /*ENable local and PLIC interrupt for UART instance */
            __enable_local_irq((int8_t)MMUART0_E51_INT);
            PLIC_EnableIRQ(MMUART0_PLIC_77);
        }
    ...
    if((&g_mss_uart1_lo == this_uart)|| ((&g_mss_uart1_hi == this_uart) ) )

    {
        if((MSS_UART_INTR_LOCAL == this_uart->intr_type) )
        {
            /* Enable local interrupt UART instance . */
            __enable_local_irq((int8_t)MMUART0_E51_INT);
            PLIC_DisableIRQ(MMUART1_PLIC);
        }
        else if((MSS_UART_INTR_PLIC == this_uart->intr_type) )
        {
            /* Enable UART instance interrupt in PLIC. */
            PLIC_EnableIRQ(MMUART1_PLIC);
            __disable_local_irq((int8_t)MMUART0_E51_INT);
        }
    ...
    if(((&g_mss_uart2_lo == this_uart) ) || ((&g_mss_uart2_hi == this_uart) ))

    {
        if((MSS_UART_INTR_LOCAL == this_uart->intr_type) )
        {
            /* Enable local interrupt UART instance . */
            __enable_local_irq((int8_t)MMUART0_E51_INT);
            PLIC_DisableIRQ(MMUART2_PLIC);
        }
        else if((MSS_UART_INTR_PLIC == this_uart->intr_type))
        {
            /* Enable UART instance interrupt in PLIC. */
            PLIC_EnableIRQ(MMUART2_PLIC);
            __disable_local_irq((int8_t)MMUART0_E51_INT);
        }
    ...
    if(((&g_mss_uart3_lo == this_uart) ) || ((&g_mss_uart3_hi == this_uart) ))

    {
        if((MSS_UART_INTR_LOCAL == this_uart->intr_type) )
        {
            /* Enable local interrupt UART instance . */
            __enable_local_irq((int8_t)MMUART0_E51_INT);
            PLIC_DisableIRQ(MMUART3_PLIC);
        }
        else if((MSS_UART_INTR_PLIC == this_uart->intr_type))
        {
            /* Enable UART instance interrupt in PLIC. */
            PLIC_EnableIRQ(MMUART3_PLIC);
            __disable_local_irq((int8_t)MMUART0_E51_INT);
        }
    ...
    if(((&g_mss_uart4_lo == this_uart) ) || ((&g_mss_uart4_hi == this_uart) ))

    {
        if((MSS_UART_INTR_LOCAL == this_uart->intr_type) )
        {
            /* Enable local interrupt UART instance . */
            __enable_local_irq((int8_t)MMUART0_E51_INT);
            PLIC_DisableIRQ(MMUART4_PLIC);
        }
        else if((MSS_UART_INTR_PLIC == this_uart->intr_type))
        {
            /* Enable UART instance interrupt in PLIC. */
            PLIC_EnableIRQ(MMUART4_PLIC);
            __disable_local_irq((int8_t)MMUART0_E51_INT);
        }
...
)
{
...

    {
        if((MSS_UART_INTR_LOCAL == this_uart->intr_type) )
        {
            /* Disable local interrupt UART instance . */
            __disable_local_irq((int8_t)MMUART0_E51_INT);
        }
        else if((MSS_UART_INTR_PLIC == this_uart->intr_type))
        {
        ...
        {
            /*Disable local and PLIC interrupt for UART instance */
            __disable_local_irq((int8_t)MMUART0_E51_INT);
            PLIC_DisableIRQ(MMUART0_PLIC_77);
        }
...
{
    if(g_uart_axi_pos & 0x01)
        MSS_UART_isr(&g_mss_uart0_hi);
    else
...
{
    if(g_uart_axi_pos & 0x02)
        MSS_UART_isr(&g_mss_uart1_hi);
    else
...
{
    if(g_uart_axi_pos & 0x04)
        MSS_UART_isr(&g_mss_uart2_hi);
    else
        MSS_UART_isr(&g_mss_uart2_lo);
...
{
    if(g_uart_axi_pos & 0x08)
        MSS_UART_isr(&g_mss_uart3_hi);
    else
        MSS_UART_isr(&g_mss_uart3_lo);
...
{
    if(g_uart_axi_pos & 0x10)
        MSS_UART_isr(&g_mss_uart4_hi);
    else
        MSS_UART_isr(&g_mss_uart4_lo);
...
)
{
    ...
    if((&g_mss_uart0_lo == this_uart))                // MMAURT 0 LOW
    {
        this_uart->hw_reg = MSS_UART0_LO_BASE;
        g_uart_axi_pos &= ~0x01;
    }

    else if(&g_mss_uart1_lo == this_uart)            // MMAURT 1 LOW
    {
    ...
    else if(&g_mss_uart2_lo == this_uart)            // MMAURT 2 LOW
    {
        this_uart->hw_reg = MSS_UART2_LO_BASE;
        g_uart_axi_pos &= ~0x04;
    }

    else if(&g_mss_uart3_lo == this_uart)            // MMAURT 3 LOW
    {
        this_uart->hw_reg = MSS_UART3_LO_BASE;
        g_uart_axi_pos &= ~0x08;
    }

    else if(&g_mss_uart4_lo == this_uart)        // MMAURT 4 LOW
    {
        this_uart->hw_reg = MSS_UART4_LO_BASE;
        g_uart_axi_pos &= ~0x10;
    }

    else if((&g_mss_uart0_hi == this_uart))            // MMAURT 0 HIGH
    {
        this_uart->hw_reg = MSS_UART0_HI_BASE;
        g_uart_axi_pos |= 0x01;
    }

    else if(&g_mss_uart1_hi == this_uart)            // MMAURT 1 HIGH
    {
        this_uart->hw_reg = MSS_UART1_HI_BASE;
        g_uart_axi_pos |= 0x02;
    }

    else if(&g_mss_uart2_hi == this_uart)            // MMAURT 2 HIGH
    {
        this_uart->hw_reg = MSS_UART2_HI_BASE;
        g_uart_axi_pos |= 0x04;
    }

    else if(&g_mss_uart3_hi == this_uart)            // MMAURT 3 HIGH
    {
        this_uart->hw_reg = MSS_UART3_HI_BASE;
        g_uart_axi_pos |= 0x08;
    }

    else if(&g_mss_uart4_hi == this_uart)            // MMAURT 4 HIGH
    {
