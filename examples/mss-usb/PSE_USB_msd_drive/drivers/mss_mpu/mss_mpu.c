...
volatile uint64_t mpu_pmp = 0;
volatile uint64_t mpu_mode = 0;
volatile uint64_t arr[10] = {0};
/***************************************************************************//**
*/
                                        ...
                                        uint8_t lock_en)
{
    ...
    if((size >= 4096u) && (0u == (size & (size - 1))) && (pmp_region < num_pmp_lut[master_port]))
    {
        ...
        mpu_mode = (MSS_MPU(master_port)->PMPCFG[pmp_region].mode);
        mpu_pmp = (MSS_MPU(master_port)->PMPCFG[pmp_region].pmp);
