 ...
 * implementation.
 *
 * This driver is based on SmartFusion2 MSS SPI driver v2.2.101
 *
 * SVN $Revision: 10267 $
 * SVN $Date: 2018-07-24 17:27:24 +0530 (Tue, 24 Jul 2018) $
 */
#include "mss_spi.h"
...
)
{
    ...
    HAL_ASSERT((this_spi == &g_mss_spi0) || (this_spi == &g_mss_spi1));
    /*
...
{
    HAL_ASSERT((this_spi == &g_mss_spi0) || (this_spi == &g_mss_spi1));
    HAL_ASSERT(frame_bit_length <= MAX_FRAME_LENGTH);
    ...
    PLIC_SetPriority( this_spi->irqn , 2);
    /* Reenable interrupts */
...
{
    HAL_ASSERT((this_spi == &g_mss_spi0) || (this_spi == &g_mss_spi1));
    HAL_ASSERT(slave < MSS_SPI_MAX_NB_OF_SLAVES);
    HAL_ASSERT(frame_bit_length <= MAX_FRAME_LENGTH);
     /* Check that the requested clock divider is within range and an even number. */
    HAL_ASSERT(clk_div >= 2u);
    HAL_ASSERT(clk_div <= 512u);
    HAL_ASSERT(0u == (clk_div & 0x00000001));
    ...
    //PLIC_SetPriority( this_spi->irqn , 3);
    /* Reenable interrupts */
    PLIC_EnableIRQ( this_spi->irqn );
   // PLIC_SetPriority( this_spi->irqn , 2);
...
)
{
    ...
    HAL_ASSERT((this_spi->hw_reg->CONTROL & CTRL_MASTER_MASK) == CTRL_MASTER_MASK);
    
    HAL_ASSERT(this_spi->slaves_cfg[slave].ctrl_reg != NOT_CONFIGURED);
    ...
    this_spi->hw_reg->FRAMESIZE = this_spi->slaves_cfg[slave].txrxdf_size_reg;
    this_spi->hw_reg->CONTROL |= CTRL_ENABLE_MASK;
...
)
{
    ...
    uint32_t rx_overflow;
    volatile uint32_t rx_fifo_empty;
