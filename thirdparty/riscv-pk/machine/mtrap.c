...
static uintptr_t mcall_shutdown(void)
{
  poweroff(0);
}
...
{
  while (1); //redirect_trap(mepc, read_csr(mstatus), read_csr(mbadaddr));
}
...
static void machine_page_fault(uintptr_t* regs, uintptr_t dummy, uintptr_t mepc)
{
  ...
  if (read_csr(mstatus) & MSTATUS_MPRV) {
    mHSS_DEBUG_PRINTF("missing redirect_trap... bah...\n");
    while (1); //return redirect_trap(regs[12], regs[13], read_csr(mbadaddr));
  }
