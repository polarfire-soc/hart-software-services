 ...
 *
 * IMPORTANT NOTE: This file has been modified from the standard HAL release to
 * include support for LwIP and FreeRTOS. If you are updating the HAL please
 * merge this version with the new version carefully.
 */
...
void handle_m_ext_interrupt(void)
{
    ...
    if((ethernet_PLIC_53 == int_num) && (0 != g_mac_context_switch))
    {
    ...
    if(((int_num >= MAC0_INT_PLIC) && (int_num <= MAC1_MMSL_PLIC)) && (0 != g_mac_context_switch))
    {
        g_mac_context_switch = 0;
        vPortYieldISR();
    }
