 ...
 *
 * SVN $Revision: 10075 $
 * SVN $Date: 2018-05-30 22:20:42 +0530 (Wed, 30 May 2018) $
 */
#ifdef PSE

#include "mss_uart.h"
#include "mss_uart_regs.h"
#include "../pse_hal/mss_sysreg.h"
#include "../pse_hal/mss_plic.h"
#include "../pse_hal/mss_hart_ints.h"
...
)
{
    ...
    if((tx_size > 0u) && ( pbuff != ((uint8_t *)0)))
    {
        ...
        if(((&g_mss_uart0_lo == this_uart) && (MSS_UART0_INTR_LOCAL == this_uart->intr_type)) ||
        	((&g_mss_uart0_hi == this_uart) && (MSS_UART0_INTR_LOCAL == this_uart->intr_type)))
        {
        	__enable_local_irq((int8_t)MMUART0_U51_INT);
        	PLIC_DisableIRQ(this_uart->irqn);
        }
        else
        {
...
)
{
    ...
    if((&g_mss_uart0_lo == this_uart))
    {
        ...
        if(MSS_UART0_INTR_LOCAL == intr_type)
        	this_uart->irqn = MMUART0_U51_INT;
        else
