...
#include "drivers/cypress_s25fl512s/cypress_s25fl512s.h"
#include "mss_plic.h"
...
{
    uint8_t command_buf[5] __attribute__ ((aligned (4))) = {CYPRESS_WRITE_ENABLE};
    uint8_t temp =0;
    ...
    command_buf[0] = CYPRESS_WR_V_CONFIG_REG;
    ...
    command_buf[0] = CYPRESS_FAST_READ;
    command_buf[1] = 0x00u;
    command_buf[2] = 0x00u;
    command_buf[3] = 0x00u;
...
{
    uint8_t command_buf[5] __attribute__ ((aligned (4))) = {CYPRESS_FAST_READ};
...
{
    const uint8_t command_buf[1] __attribute__ ((aligned (4))) = {CYPRESS_READ_STATUS_REG};
...
)
{
    ...
    uint8_t command_buf[6] __attribute__ ((aligned (4)));
   
    command_buf[0] = S25Fl512S_READ_DATA;
    command_buf[1] = (page >> 24u) & 0xFFu;
    command_buf[2] = (page >> 16u) & 0xFFu;
    command_buf[3] = (page >> 8u) & 0xFFu;
    command_buf[4] = page & 0xFFu;
    command_buf[5] = 0xFFu;
    ...
    MSS_QSPI_polled_transfer_block(0, command_buf, 5, buf, length, 0);
...
)
{
     ...
     * Write Enable
     */
    command_buf[0] = CYPRESS_WRITE_ENABLE;
    MSS_QSPI_polled_transfer_block(0, command_buf, 0, (uint8_t*)0, 0,0);
     ...
     * Program page buffer
     */
    command_buf[0] = CYPRESS_PAGE_PROGRAM;
...
//    MSS_QSPI_polled_transfer_block(2, command_buf, wr_len + 2, (uint8_t*)0, 0,0);
     ...
     */
    command_buf[0] = S25Fl512S_PROGRAM_EXECUTE;
    command_buf[1] = 0u;
...
)
{
    ...
    mHSS_PRINTF(CRLF "program_block called: buf = %p, wr_addr = %u, wr_len = %u" CRLF, buf,
        wr_addr, wr_len);

    ASSERT(wr_len <= BLOCK_LENGTH);
    mHSS_PRINTF(CRLF "program_block called: page_nb1 = %u" CRLF, page_nb);

//    erase_block(page_nb);

#ifdef PAGE_LENGTH_HACK
    erase_block(page_nb / 4);
    erase_block(page_nb / 4 + 0x40000);
    mHSS_PRINTF(CRLF "program_block called: page_nb2 = %u" CRLF, page_nb);
#else
    erase_block(page_nb);
    mHSS_PRINTF(CRLF "program_block called: page_nb3 = %u" CRLF, page_nb);
#endif
    mHSS_PRINTF(CRLF "program_block called: page_nb4 = %u" CRLF, page_nb);
...
{
    mHSS_PRINTF(CRLF "Flash_program called: buf = %p, wr_addr = %u, wr_len = %u" CRLF, buf,
        wr_addr, wr_len);
...
static void disable_write_protect(void)
{
    ...
    command_buf[0] = S25Fl512S_WRITE_STATUS_REG;
...
static void erase_block(uint32_t page_nb)
{
    ...
    mHSS_PRINTF(CRLF "erase_block called: page_nb = %u" CRLF, page_nb);
    wait_for_wip();
     ...
     * Erase
     */
    command_buf[0] = S25Fl512S_BLOCK_ERASE;
    command_buf[1] = (page_nb >> 24u) & 0xFFu;
    command_buf[2] = (page_nb >> 16u) & 0xFFu;
    command_buf[3] = (page_nb >> 8u) & 0xFFu;
    command_buf[4] = page_nb & 0xFFu;

    MSS_QSPI_polled_transfer_block(0, command_buf, 5, (uint8_t*)0, 0,0);
