...
void RunStateMachines(void) 
{
            ...
            //mHSS_DEBUG_PRINTF("running machine %s" CRLF, pMachineName);
            {
                ...
                // debug print any state transitions...
                {
...

                    {
                        ...
                        if (prevState != currentState) {
                            mHSS_DEBUG_STATE_TRANSITION_TEXT;
...
#if defined(CONFIG_DEBUG_LOOP_TIMES) || defined(CONFIG_DEBUG_IPI_STATS)
    {
        ...
        if (delta > maxLoopTime) { maxLoopTime = delta; max_exceeded_flag = true; }

        if ((loopCount % (unsigned long)CONFIG_DEBUG_LOOP_TIMES_THRESHOLD) == 0u) {
            dump_flag = true;
        }

        if (dump_flag || max_exceeded_flag) {
#ifdef    CONFIG_DEBUG_PROFILING_SUPPORT
            dump_profile();
#endif
            if (dump_flag) {
