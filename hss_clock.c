...
#else
    __asm volatile ("rdtsc" : "=A"(tickCount));
#endif
    return tickCount;
}
...
{
    //mHSS_DEBUG_PRINTF("Start: %u, Duration: %u" CRLF, startTick, durationInTicks);
    //return (HSS_GetTickCount() > (startTick + durationInTicks));
    return (HSS_GetTime() > (startTick + durationInTicks));
}

void HSS_SpinDelay_MilliSecs(uint32_t milliseconds)
{
    //HSSTicks_t delayTick = HSS_GetTickCount();
